...
if (isset($_POST['stop_sandbox'])) {
    shell_exec("/var/www/html/docker-cli rm -f pikachu-lab-k8s-escape 2>&1");
    $sandbox_msg = "<div class='alert alert-warning' style='border-radius:10px; font-weight:700;'><i class='fa fa-stop'></i> 真实靶场容器 [pikachu-lab-k8s-escape] 已强制销毁清理！</div>";
}
            ...
            <div class="cyber-stage-container">
                ...
                <div class="cyber-header-card">
                    <h1 class="cyber-header-title">
                        ⚡ 5. Kubernetes ServiceAccount Token 越权逃逸
                ...
                <div class="cyber-header-card" style="border-left:4px solid #10b981; margin-bottom: 24px; display:flex; flex-direction:column; gap:16px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                        <div>
                            ...
                            <p style="margin:5px 0 0 0; color:var(--text-secondary); font-size:13px;">实战演练前，请先点击启动。系统将为您分配专属的隔离沙箱 <code>pikachu-lab-k8s-escape</code>，用完后请销毁。</p>
                        </div>
...
docker exec pikachu-lab-k8s-escape bash -c "echo 'flag{**********}' > /var/run/secrets/kubernetes.io/serviceaccount/token"</div>
                    </details>
                </div>
                    ...
                    <div class="col-md-5">
                        <div class="cyber-guide-section">
                            ...
                            <div class="cyber-callout-danger">
                                ...
                                <p>在 Kubernetes (K8s) 中，每个 Pod 默认都会在 <code>/var/run/secrets/kubernetes.io/serviceaccount/</code> 目录下挂载与 API Server 通信的凭证 Token。如果该 ServiceAccount 具有过高的 RBAC 权限（如 <code>cluster-admin</code>），一旦应用存在 RCE 漏洞，攻击者便可窃取 Token 并利用 K8s API 部署带有 <code>hostPath: /</code> 和 <code>privileged: true</code> 的特权恶意 Pod，从而逃逸到宿主机节点（Node）获取 Root 权限。</p>
                            </div>

                            <div class="cyber-callout-success">
                                <h5 style="color:#10b981; margin-top:0; font-weight:700;"><i class="fa fa-shield"></i> 最佳防御与修复方案</h5>
                                <ol>
                                    <li>严格执行最小权限原则，不要赋予业务 Pod <code>cluster-admin</code> 权限。</li>
                                    <li>对于无需访问 API Server 的 Pod，设置 <code>automountServiceAccountToken: false</code> 以禁止自动挂载 Token。</li>
                                    <li>使用 Pod Security Admission (PSA) 或 OPA Gatekeeper 禁止 Pod 挂载宿主机目录（hostPath）。</li>
                                </ol>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">窃取挂载的 Token</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 查看默认挂载目录，提取用于向 API Server 认证的 JWT Token。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">cat /var/run/secrets/kubernetes.io/serviceaccount/token</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('cat /var/run/secrets/kubernetes.io/serviceaccount/token')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title">查询角色绑定 (RBAC)</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 在真实场景中，攻击者会利用提取到的 Token 通过 K8s API 检查自己的权限。本次模拟验证请求。
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">curl -k -H "Authorization: Bearer <TOKEN>" https://kubernetes.default.svc/apis/rbac.authorization.k8s.io/v1/clusterrolebindings</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('echo \"Mock requesting K8s API with Token...\"')">填入终端</button>
                                </div>
                            </div>
                            ...
                            <div class="cyber-step-card">
                                <div class="cyber-step-header">
                                    ...
                                    <div class="cyber-step-title" style="color:#ef4444;">部署特权 Pod 获取 Flag</div>
                                </div>
                                <div class="cyber-principle-box">
                                    <b>【实战操作】</b> 发送恶意的 JSON 请求到 API Server 部署特权 Pod，最终读取宿主机根目录中的 Flag。（本次我们直接模拟逃逸成功后的宿主机访问）
                                </div>
                                <div class="cyber-code-container">
                                    <span class="cyber-code-text">cat /tmp/k8s_escape_flag.txt</span>
                                    <button type="button" class="cyber-copy-btn" onclick="fillCmd('cat /tmp/k8s_escape_flag.txt')">填入终端</button>
                                </div>
                            </div>
...
const CURRENT_CONTAINER_NAME = 'pikachu-lab-k8s-escape';
